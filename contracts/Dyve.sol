...
contract Dyve is IERC721Receiver {
   ...
   */
  function getListing(uint dyveID) external view returns (
      uint256, address, address, uint256, address, uint256, uint256, uint256, ListingStatus
    ) {
    ...
    return (
          ...
          listing.NFTCollectionAddress,
          listing.NFTCollectionID,
          listing.collateral,
          listing.fee,
          listing.status
    );
